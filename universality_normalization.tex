...
\]
The relative-entropy formula holds if \(s_2=c/12\), which is a conjecture (Correction~C3).  For a two-dimensional CFT with central charges \((c,\bar c)\) the chiralities add with opposite rotations: \(-\log F^{(\lambda)}=\Phi_c\bigl(z(1+e^{-\pi\lambda})\bigr)+\Phi_{\bar c}\bigl(z(1+e^{\pi\lambda})\bigr)\), even in \(\lambda\), with a minimum at \(\lambda=0\) proved only at leading order (the law itself is proved only for the massless Dirac fermion; Correction~C3).  In the lattice variables of Vardhan--Wei--Zou (\(\lambda=2t\), \(\eta\) their cross ratio, ordinary Petz map \(\lambda=0\)):
\[
...
\begin{enumerate}
\item \emph{Lemma~\ref{lem:second-variation}} (type-III second variation) for Theorem~B.  Status: the fidelity (Bures) half is proved for every von Neumann algebra with cyclic separating vector (\texttt{rigor/lemma\_second\_variation.tex}), and Theorem~B itself is established for every diffeomorphism covariant net on \(S^1\) (\texttt{rigor/universality\_theorem\_B.tex}, \texttt{rigor/implementation\_C11.tex}, refereed): the implementer \(U_s=e^{isT(g_{\rm tot})}\) exists by the Carpi--Weiner energy bound because \(g_{\rm tot}\) is \(C^{1,1}\) with a single jump of the second derivative, and \(c\)-linearity of \(g_B\) follows because the stress-tensor subspace reduces \(\Delta\) and \(J\).  The Kubo--Mori half is proved for unitary orbits whose generator \(A\) is bounded in the algebra, or affiliated with it with \(\Omega\in D(A)\) (\texttt{rigor/kubo\_mori\_second\_variation.tex}: exact identity and lower bound, and the second-order expansion under its condition \((U_\varepsilon)\), which modular analyticity implies only for bounded \(A\); see Correction~C1 at the end); for \(T(g_{\rm tot})\), which is affiliated only with a larger algebra and not with \(\cA(I)\), this is open (Gap~QK2 there) and a gauge lemma is still missing.  For the free fermion both are bypassed.
...
{\sloppy
These corrections implement the knowledge-base cards \texttt{km-exact-identity} (C1), \texttt{conj-s2} (C2) and \texttt{two-chirality-minimum-at-lambda-zero} with \texttt{two-chirality-law-general-cft} (C3; C2 and C3 added in a fourth round) (project
\texttt{cft\_cmi}), whose refereed Statements are the specification.  Each changed line was replaced by
exactly one line, so line numbers cited elsewhere still point to the same text.
\paragraph{C1. The Kubo--Mori half for affiliated generators (Section~\ref{sec:remains}, item~1,
line~427).}
\emph{Before:} ``The Kubo--Mori half is proved for unitary orbits with generator affiliated with the
algebra (\texttt{rigor/kubo\_mori\_second\_variation.tex})''.
\emph{Now:} it is proved for unitary orbits whose generator \(A\) is bounded in the algebra, or
affiliated with it with \(\Omega\in D(A)\): the exact identity
\(D(\omega\Vert\omega_s)=\|h(\Delta)^{1/2}(e^{isA}-1)\Omega\|^2\), the sum rule and the lower bound
\(\liminf s^{-2}D\ge\frac12g_{\mathrm{KM}}\) hold, and the second-order expansion holds under the
condition \((U_\varepsilon)\) of that document.  Modular analyticity of \(A\) implies
\((U_\varepsilon)\) only for bounded \(A\).  A generator affiliated only with a larger algebra, such as
\(T(g_{\rm tot})\), is not covered; that case is open (Gap~QK2 of
\texttt{rigor/kubo\_mori\_second\_variation.tex}).
...
\((U_\varepsilon)\); for unbounded \(A\) the modular-analyticity route \((A_\varepsilon)\Rightarrow
(U_\varepsilon)\) is not available (lem:AtoU is for bounded \(A\)).  (c) Not applicable to the
recovery curve of Note~7, whose generator \(T(g_{\rm tot})\) is affiliated with the global algebra but
not with the interval algebra, so \(e^{isA}\) is not in \(M\) (Gap QK2, l.~822)''.  The statement
at line~277 that the Kubo--Mori half of Lemma~\ref{lem:second-variation} remains open beyond finite
dimensions is unchanged: that lemma allows generators affiliated with a larger algebra, which is
exactly the open case.
\paragraph{C2. The Kubo--Mori bracket (line~301; added in a fourth round).}
\emph{Before:} ``the rigorous bracket for general nets is only \(0.034c\le\liminf s^{-2}D\le\limsup
s^{-2}D\le0.924c\) (the latter under a uniform-integrability hypothesis)''.
\emph{Now:} the bracket is conditional at both ends: the upper end under a uniform-integrability
hypothesis that is open, the lower end modulo Araki's perturbation theory, whose sources were not obtained.
...
\paragraph{C3. The rotated family and the relative entropy (Corollary~\ref{cor:law}: lines~411 and~416, on the displays at lines~409 and~414; added
in a fourth round).}
\emph{Before:} ``\(D(\omega\Vert\widetilde\omega_t)=\frac{c}{12}\zeta_t^2+o(\zeta_t^2)\)'', ``even in \(\lambda\) and
minimal at \(\lambda=0\)'', ``\(=0.667\,c\,\eta^2\)'' and ``for a two-dimensional CFT the ordinary Petz map is the optimum
of the family''.
\emph{Now:} the relative-entropy statements hold if \(s_2=c/12\), which is a conjecture; the minimum at \(\lambda=0\), and
with it the optimality of the ordinary Petz map within the family, is proved only at leading order in the cross ratio;
the two-chirality law itself is proved only for the massless Dirac fermion.
\emph{Why:} card \texttt{conj-s2} (conjectural); card \texttt{two-chirality-minimum-at-lambda-zero} (conjectural: beyond the
leading order ``No proof exists''); card \texttt{two-chirality-law-general-cft} (conjectural; the proved special case is
the massless Dirac fermion, card \texttt{two-chirality-law}).
\par}
\end{document}
