...
a rigorous bound, certified error bars, and the rate}
...
\item \emph{A structural tail bound with no kernel estimates} (Theorem~\ref{thm:main}, \S\ref{sec:tail}):
 ...
 \]
 with $\Psi$ the explicit triangle-inequality function \eqref{eq:Psi}; to leading order
...
\item \emph{An occupation-tail estimate with a controlled remainder}
 ...
 \[
  ...
  \ \le\ \frac{C_2\,\zeta^{2}}{\kappa_c^{3}}\ +\ C_1\,\zeta\,\kappa_c\,e^{-\kappa_c},
 \]
 with $C_1\le0.051$ and $C_2\le0.46$ (both certified numerically in \S\ref{sec:num}; the analytic
...
\item \emph{Consequences} (\S\ref{sec:tail}, \S\ref{sec:rate}): $\Phi-\Phi_W\le C\,\zeta^2/\kappa_c$
 with an explicit $C$ --- one power of $\kappa_c$ short of the conjectured
 ...
 Note~5, Cor.~4.2.
\end{enumerate}
Sections~\ref{sec:box}--\ref{sec:bars} turn (A)+(B) into certified error bars for the tabulated values,
...
are finite; numerically $A_3\le0.036$ and $A_1\le2.0$, i.e.
\[
 \boxed{\ C_2\le0.46,\qquad C_1\le0.051\ }
\]
(\S\ref{sec:num}).  Consequently, with $\nu_+=\log(1+e^{-\kappa_c})\le e^{-\kappa_c}$,
\begin{equation}\label{eq:taunu}
 5\tau+\nu\ =\ 12\,\nu_++10\,\Sigma(\kappa_c)
 \ \le\ \frac{2\zeta^2}{3\kappa_c^2}+\frac{4.6\,\zeta^2}{\kappa_c^3}
...
constants $A_1,A_3$ are proved finite but their numerical values ($A_3\le0.036$, $A_1\le2.0$) are
...
\]
Then $\Phi_W\le\Phi$ and, provided $y:=2e^{2\Phi_\partial}\sqrt{\Phi\,\Phi_\partial}<1$,
...
\end{equation}
With Theorem~\ref{thm:tau}, $\mathcal T\le\frac{2\zeta^2}{3\kappa_c^2}+\frac{4.6\zeta^2}{\kappa_c^3}
+\bigl(12+0.51\zeta(1+\kappa_c)\bigr)e^{-\kappa_c}$.
\end{theorem}
...
$\langle1\rangle1$}
\lstep{1}{5}{$e^{-\Phi}\ge e^{-\Phi_W}\bigl(e^{-\Phi_\partial}-y'\bigr)$ with
$y':=2e^{\Phi_\partial}\sqrt{\Phi\Phi_\partial}$, hence
$\Phi-\Phi_W\le-\log(e^{-\Phi_\partial}-y')=\Phi_\partial-\log(1-y'e^{\Phi_\partial})$
and $y'e^{\Phi_\partial}=y$.}
...
\]
For the free fermion one may take $f=0.0085$ (Note~5, Table~3, all tabulated $\Phi/\zeta^2\le0.00838$,
and $\lim_{\zeta\to0}\Phi/\zeta^2=1/(12\pi^2)=0.008444$ by \texttt{uhlmann\_upper\_bound.tex} +
\texttt{quadratic\_limit.tex}), giving
\begin{equation}\label{eq:final}
 \Phi(\zeta)-\Phi_W(\zeta)\ \le\ \frac{0.173\,\zeta^2}{\kappa_c}\;+\;\frac{0.88\,\zeta^2}{\kappa_c^2}.
\end{equation}
\end{corollary}
\lstep{1}{1}{$\mathcal T\le\frac{\zeta^2}{\kappa_c^2}\bigl(\frac23+\frac{4.6}{\kappa_c}\bigr)+0.6\frac{\zeta^2}{\kappa_c^2}
+0.51\zeta(1+\kappa_c)e^{-\kappa_c}\le\frac{1.75\zeta^2}{\kappa_c^2}$ for $\kappa_c\ge10$.}
\lby{Theorem~\ref{thm:tau} \eqref{eq:taunu}; the hypothesis on $\kappa_\dagger$ gives
$e^{-\kappa_c}\le e^{-3}\zeta^2(1+\kappa_c)^{-2}$, so $12e^{-\kappa_c}\le0.6\zeta^2\kappa_c^{-2}$ and
$0.51\zeta(1+\kappa_c)e^{-\kappa_c}\le0.51e^{-3}\zeta^3(1+\kappa_c)^{-1}\le0.003\zeta^2\kappa_c^{-2}$;
$\frac23+0.46+0.6+0.003\le1.75$}
\lstep{1}{2}{$\Phi_\partial\le\frac{\mathcal T}{2(1-\mathcal T)}\le\frac{0.875\zeta^2}{\kappa_c^2}(1+O(\zeta^2))$.}
\lby{$\langle1\rangle1$ and $\mathcal T\le1.75\zeta^2/100\le0.0175$}
\lqed{1}{3}\lby{Theorem~\ref{thm:main}, $\langle1\rangle2$, $\Phi\le f\zeta^2$, and
$2\sqrt{\Phi\Phi_\partial}\le2\sqrt{f\zeta^2\cdot0.875\zeta^2\kappa_c^{-2}}
=2\sqrt{0.875f}\,\zeta^2/\kappa_c$; with $f=0.0085$, $2\sqrt{0.875\cdot0.0085}=0.1725$}

\begin{verdictgap}
Proved: an unconditional tail bound $\Phi-\Phi_W\le C\zeta^2/\kappa_c$ with the explicit constant
$C=0.173$ (plus a $0.88\zeta^2\kappa_c^{-2}$ term), valid once the window reaches
$\kappa_c\ge2\log(1/\zeta)+2\log\kappa_c+3$.  \emph{Not} proved: the conjectured
...
bound on $\Phi_E-\Phi_{P_{\rm sub}}$.  Empirically it is small: Note~5, Table~1 varies
...
labelled ``certified modulo the box discretisation''.
...
\section{Certified error bars}\label{sec:bars}
...
\paragraph{The bar.}  Combining (a), (b), \eqref{eq:final} and \eqref{eq:arith}: with
...
(coefficient $1/15$ or $0.0641$ according to \S\ref{sec:box}c), whose own proved uncertainty is
$C_2\zeta^2\kappa_{\rm eff}^{-3}\le0.46\,\zeta^2\kappa_{\rm eff}^{-3}$ ($=0.6\,\%$ of $\Sigma$ at
...
\caption{Certified bar \eqref{eq:bar} (units $10^{-5}$) from the raw sub-compressed values of
...
$\zeta$ & $10^5\Phi^{\rm c}_{\rm sub}$ & certified interval & $10^5\,$best & width/$\Phi$\\\midrule
...
\begin{verdictgap}
The certified bar is a factor $1.8$--$2.5$ wide, i.e.\ far weaker than the heuristic $0.3$--$1\,\%$ of
...
$0.0641\zeta^2\kappa_{\max}^{-2}$), is the exact leading ultraviolet occupation tail, with a proved
relative error $\le0.6\,\%$ at $\kappa_{\rm eff}\simeq30$ --- Note~5's heuristic ``12--56\,\% off''
...
The rate $\Phi=f_2\zeta^2[1+O(\log^{-2}(1/\zeta))]$ of Note~5, Cor.~4.2, was to follow by combining
...
\lby{Theorem~\ref{thm:main}, Corollary~\ref{cor:rate}, \eqref{eq:cubic} applied to $P\uparrow E$
...
(which is genuine, cf.\ Note~5, Cor.~4.2) into a \emph{linear} rather than quadratic penalty.  A
...
$\kappa_c=6,8,12,18.4,25.3$ (beyond that the double-precision floor is reached): $A_1=2$ exactly, so
...
=0.0355,\,0.0326,\,0.0275,\,0.0214$ at $\zeta=0.05,0.2,0.5,1$ (monotone decreasing in $\zeta$), so
$A_3\le0.036$ and $C_2=4\pi A_3\le0.46$.
...
& \multicolumn{3}{c}{$\bigl[\Sigma-\zeta^2/(15\kappa_c^2)\bigr]\kappa_c^3/\zeta^2$ (bound: $\le0.46$)}\\
...
The proved remainder bound $0.46\,\zeta^2\kappa_c^{-3}$ holds with a factor $6$--$30$ to spare, and
...
$\Sigma=3.3\times10^{-7}$: the certified bound is a factor $\approx80$ above the truth, entirely
...
\item \textbf{Proved (unconditional).}  $\Phi-\Phi_W\le0.173\,\zeta^2\kappa_c^{-1}+0.88\,\zeta^2\kappa_c^{-2}$
for $\kappa_c\ge\max\{10,\ 2\log\frac1\zeta+2\log(1+\kappa_c)+3\}$ (Theorem~\ref{thm:main},
Corollary~\ref{cor:rate}).  The route is: an exact factorisation making $\Phi_W$ the Bures distance to
...
\item \textbf{Proved.}  The occupation tail: $\tau_+=\tau_-$ exactly, and
$\bigl|\int_{\kappa_c}^\infty(\widetilde Q(\kappa,\kappa)-q_\kappa)\dd\kappa-\frac{\zeta^2}{15\kappa_c^2}\bigr|
\le0.46\,\zeta^2\kappa_c^{-3}+0.051\,\zeta(1+\kappa_c)e^{-\kappa_c}$ (Theorem~\ref{thm:tau}).  This
...
$\Phi=f_2\zeta^2[1+O(\log^{-1}(1/\zeta))]$ --- still one logarithm short of Note~5, Cor.~4.2
(Proposition~\ref{prop:rate}).
\item \textbf{Certified numerics.}  Table~\ref{tab:bars}: the tabulated $\Phi_{\rm sub}$ are rigorous
lower bounds (up to an arithmetic margin of $\approx3\,\%$, dominated again by a cross term and
removable by an interval-arithmetic audit of $\widehat D$ and $Q_N$); the certified upper bound is a
factor $1.8$--$2.5$ above, and is in addition conditional on the box discretisation
(\S\ref{sec:box}b).  The \emph{correction} $\zeta^2/(15\kappa_c^2)$ that Note~5 adds is now proved
correct to $0.6\,\%$ at $\kappa_c\simeq30$; what is not proved is that the correction to $\Phi$ equals
