...
\begin{center}\shot[.92\textwidth]{V3_Araki77_thm38.png}\\[2pt]
\shot[.92\textwidth]{V3_Araki77_thm39.png}\end{center}
...
\begin{center}\shot[.93\textwidth]{V3_VWZ_lambda.png}\end{center}
...
which is \emph{even} in $t$ (hence in $\lambda$); that it is minimised at $t=0$ is proved only at leading order, since step $\langle1\rangle8$ below uses the false ``$\Phi'$ nondecreasing'' (card phi-derivative-nondecreasing; Correction~C1).
\end{proposition}
...
$g(-t)=g(t)$, $g'(0)=0$, and $g'(t)>0$ for $t>0$ [the last claim is not proved: Correction~C1].}
...
{\sloppy
This correction implements the knowledge-base card \texttt{two-chirality-minimum-at-lambda-zero} (project
\texttt{cft\_cmi}), at the user's request of 2026-09-24.  Each changed line (1378, 1413) was replaced by exactly one line,
so line numbers cited elsewhere still point to the same text.
\paragraph{C1. The minimum at \(t=0\) (Proposition~\ref{prop:chirality}, line~1378, and step~\(\langle1\rangle8\), line~1413).}
\emph{Before:} ``which is \emph{even} in \(t\) (hence in \(\lambda\)) and minimised at \(t=0\)'', with step
\(\langle1\rangle8\) claiming \(g'(t)>0\) for \(t>0\).
\emph{Now:} the evenness is proved; the minimum at \(t=0\) is proved only at leading order.
\emph{Why:} step \(\langle1\rangle8\) uses that \(\Phi'\) is nondecreasing, which is false (card
\texttt{phi-derivative-nondecreasing}, refuted); card \texttt{two-chirality-minimum-at-lambda-zero} (conjectural) records the
minimum as open beyond the leading order, where the bracket \(2+4\cosh\pi\lambda+2\cosh2\pi\lambda\) is strictly increasing
in \(|\lambda|\), and notes that it would follow from the unproved property that \(\zeta\Phi'(\zeta)\) is nondecreasing.
\par}
