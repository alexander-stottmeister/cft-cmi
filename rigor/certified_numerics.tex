...
\label{sec:scope}
% filled in last
This note has two independent parts, both aimed at the width of the certified brackets for
$\Phi(\zeta)=-\log\Fid(\omega,\widetilde\omega_s)$ of Note~5, Table~3, which
\texttt{tail\_bound.tex} (agent PT) left at $82$--$154\,\%$.
...
\item Consequently (Theorem~\ref{thm:nocross}) the ultraviolet tail bound can be freed of the Bures
 ...
 through hypothesis \eqref{eq:Hyp} has a measured constant $c_0\in[18,42]$ and is four to six times
 weaker than the computable form (\S\ref{sec:num}).
\end{itemize}
Numerically this replaces the factor-$80$ overshoot of \texttt{tail\_bound.tex} by a factor
$50$--$160$; the resulting widths are stated in \S\ref{sec:rcr}, which supersedes
...
\S\ref{sec:num} the right-hand side at $\zeta=1/15$, $\kappa_c=32.2$ is $\approx3\times10^{-7}$
instead of the $2.77\times10^{-5}$ of \eqref{eq:final} of \texttt{tail\_bound.tex} --- a factor $90$,
and $0.9\,\%$ of $\Phi$.  The price is hypothesis \eqref{eq:Hyp}, an operator inequality between
computable matrices, which \S\ref{sec:num} verifies with $c_0,c_1\le1.02$.
\end{remark}
...
\texttt{results\_A2.txt}/\texttt{results\_hp2.txt}).
...
observed $2.8$--$5.5$), confirming the exponent upgrade.
...
$20$--$30\,\%$ of $\Phi_W$: still a factor $4$--$6$ better than \eqref{eq:final}, but the sharp
...
\begin{verdictok}
The certified brackets for the box value are $0.6$--$1.2\,\%$ wide, and $1.7$--$2.4\,\%$ wide for
$\Phi$ itself once PT's box-cutoff estimate is added; the previous certified widths were
...
\item $Q_N$ is now enclosed to $5.6\times10^{-18}$ per entry in ball arithmetic from $2N$ certified
 ...
 $4.7\times10^{-11}$, an eight-percent (not three-percent) arithmetic margin.
...
systematically \emph{smaller} than the double-precision value (e.g.\ $2.13\times10^{-9}$ against
$2.60\times10^{-9}$ at $\zeta=1/120$) and tracks $\Phi_{22}$ closely, $\Phi_{\rm off}/\Phi_{22}\in
[0.83,1.10]$, as the structure of \eqref{eq:computable} predicts (both are $O(\tau)$).  (iii) The
enclosures are narrower, not wider: the corrected upper ends lie below the previous ones.
