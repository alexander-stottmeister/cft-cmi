...
- Kubo-Mori: gauge lemma REFUTED 2026-09-09 (rigor/kubo_mori_gauge_lemma.tex): inf over admissible gauges = 11.09 c/6; proved bracket 0.034c <= liminf <= limsup <= 0.924c [corrected 2026-09-24: not proved outright; both ends are conditional, the upper end on hypothesis (V), open (Gap RK1), the lower end on Araki's perturbation expansion, whose sources were not obtained (Gap RK2); status table of rigor/kubo_mori_gauge_lemma.tex]; s_2 = c/12 for general nets stays conjectural (numerically confirmed for fermion and boson). Rate of the remainder: rigor/rate_of_remainder.tex, REFEREED (RRr): global bound Phi <= -(1/2)log(1-2 f_2 zeta^2) unconditional via the group extension [corrected 2026-09-24: proved for zeta < pi sqrt(3) = 5.44 (card rrr-bound-small-zeta); on pi sqrt(3) <= zeta < pi sqrt(6) it rests on the index-zero cards pushforward-blocks-equal-singular-values (heuristic) and group-index-zero-large-s (conjectural); rigor/rate_of_remainder.tex Correction C3], c_3 = -0.99(1), c_4 = +0.9(1) [corrected 2026-09-24: computed values whose scripts are not in the repository, and the integer-power expansion is not proved; card remainder-c3-c4-values, heuristic], log^-2 claim refuted; Note 5 Cor 4.2, Note 7, paper 1 corrected. Certified numerics: rigor/certified_numerics.tex, REFEREED (RCr): theory right, Phi_off column was float64 noise; certified: Q_N enclosure (5.62e-18/entry); a priori Phi_W <= Phi^box <= 1.20 Phi_W [corrected 2026-09-24: not certified: it holds modulo the Dhat quadrature and the box cutoff, with the measured constants 18 <= c_0 <= 42 and c_1 <= 1.011 of its hypothesis; card certified-numerics, open_problems_plan.tex C2]; 60-digit enclosures of the box value with widths 0.71-1.01% (zeta = 1/120 ... 8/15, kappa_c = 25.3), repairs (1)-(6) applied (Sec. 9). Not certified: Dhat quadrature, box cutoff beyond kappa_max.
