...
| findings.tex | 39 | Consolidated findings by severity + status tables + errata (assembled from findings_entries.tex) |
...
| uhlmann_upper_bound.tex | 35 | PA (Phase 1): Uhlmann–Bogoliubov upper bound ⇒ lim Φ/ζ² = c/(12π²) (Thms 8.1-8.2); refereed by PR, repairs R1-R5; §10 proves (H2),(H3) for the fermion |
...
| tail_bound.tex | 22 | PT (Phase 1, O4): tail bound Φ−Φ_W ≤ Cζ²/κc + C′ζ²/κc², shape proved (Thm 5.1, Cor 5.2); the explicit constants 0.173ζ²/κc + 0.88ζ²/κc² (Cor 5.2, eq:final) rest on numerical inputs (A₁ ≤ 2.0, A₃ ≤ 0.0374 in double precision, not certified; f = 0.0085), Prop 4.1 upgraded (Thm 4.2), error brackets not certified (upper half from eq:final); log⁻² rate obstructed |
| universality_theorem_B.tex | 25 | QA (Phase 2): Theorem B (Thm 7.1) for all diffeo-covariant nets with (H1)-(H3)+(H2'); c-linearity via H_T; refereed by QR1, repairs applied (Sec. 9) |
...
| rate_of_remainder.tex | 20 | RR (Phase 2b): global bound Phi <= -(1/2)log(1-2 f_2 zeta^2) UNCONDITIONAL (group extension, PA Prop 7.6) [corrected 2026-09-24: proved for zeta < pi sqrt(3) = 5.44 (card rrr-bound-small-zeta); on pi sqrt(3) <= zeta < pi sqrt(6) it rests on the index-zero cards pushforward-blocks-equal-singular-values (heuristic) and group-index-zero-large-s (conjectural); rigor/rate_of_remainder.tex Correction C3], c_3 = -0.99(1) [corrected 2026-09-24: computed values whose scripts are not in the repository, and the integer-power expansion is not proved; card remainder-c3-c4-values, heuristic], log^-2 claim refuted; refereed by RRr, repairs applied (§6.2-6.4) |
| certified_numerics.tex | 16 | RC (Phase 2b): weighted Sylvester bound, cross term removed; 60-digit enclosures of the box value 0.71-1.01%; a priori 1.20 Phi_W; refereed by RCr, repairs applied (Sec. 9) |
...
| optimality_second_order.tex | 16 | S8 (Phase 3, O8): quasi-free second-order problem is an exact SDP; optimum = geometric-compression value (Petz off by 4); A-C lower bound: (Tcond) ⇒ E_AC = 0 (Prop 4.2), so E_AC = 0 on the three hopping chains (4,8,2), (4,12,2), (4,16,2), where (Tcond) holds numerically; not a general zero; Petz not optimal, compression unproved |
| optimality_all_channels.tex | 14 | Orchestrator (Phase 3b): every recovery channel's recovered symbol is quasi-free feasible (Kadison–Schwarz + Tomita + BJL); exact single-observable fidelity bound; all-channel second-order optimum = quasi-free SDP value (Sion); §5: S10's exact KKT certificate + isometric-orbit refutation, exact-fidelity confirmation, independent Galerkin cross-check of θ. VERDICT: compression not the minimiser of the second-order functional (θ≥0.30); optimality in exact fidelity OPEN (upper half for the rotated channel), E_rec ∈ [D_z, f₂z²] |
| sdp_dual_certificate.tex | 17 | S10 (Phase 3b): exact necessary-and-sufficient KKT certificate at the compression (unique multipliers, V_c unitary); Legendre reduction V(t)=‖P⊥tP‖²; Möbius rigidity δ_c,DD=0 ⇒ certificate ⇔ E_D u E_D = 0; θ = 0.300(5) numerically ⇒ compression∘e^{−εG*} beats compression by 1−θ; route 2 closed |
| sdp_certificate_numerics.tex | 11 | S11 (Phase 3b): lattice strong duality/KKT verified (4,8,2); block-exact modular Galerkin discretisation (closed-form Q via Li₂, reproduces f₂ to 0.3%); certificate test at compression structurally inconclusive in finite models; exact windowed fidelity confirms S10's gain (ratio → 0.67) |
...
| exact_fidelity_upper_half.tex | 23 | E1 (Phase 4): exact-fidelity upper half for isometric quasi-free channels via arbitrary exterior extensions [corrected 2026-09-24: arbitrary subject to two Hilbert-Schmidt conditions, on both off-diagonal blocks of the extended unitary; the determinant bound also needs index zero, which holds for the small-s family used, so the result is unaffected; card uhlmann-upper-half-proved, exact_fidelity_upper_half.tex C1, C3]; THEOREM closure(Ext) = N^perp (generic position / twisted duality); E_rec <= (1 - theta_fr) f2 z^2 (1+o(1)), theta_fr > 0 iff E_D u E_D != 0; sandwich Cor 7.5; refereed (referee_exact_fidelity_upper_half.tex, stands with repairs Sec. 10) |
