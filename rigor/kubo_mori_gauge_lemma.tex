...
\item evaluate the right-hand side to second order under hypothesis $(\mathrm V)$ of Definition~\ref{def:V}, which is open ($(\mathrm U_\eps)$ for $\cN$, the brief's choice, fails by Proposition~\ref{prop:Afails}), obtaining
...
\begin{equation}\label{eq:mainbound}
 ...
 \qquad\text{(conditional on $(\mathrm V)$, open; Correction~C1)},
\end{equation}
...
$(\mathrm U_\eps)$ and $(\mathrm A_\eps)$ (modular analyticity of the generator) \emph{fail} for every field compactly supported
 in $K$ & \textbf{proved} (Prop.~\ref{prop:Afails})\\
$(\mathrm V)$ at $\sigma=0$, i.e.\ for the single tangent vector $T(\hat g)\Om$
 & \textbf{proved} (Prop.~\ref{prop:V0})\\
$(\mathrm V)$ along the orbit & \textbf{open} (Gap~RK1)\\
...
{\sloppy
This correction implements the knowledge-base card \texttt{conj-s2} (project \texttt{cft\_cmi}), whose
Statement is the specification, at the user's request of 2026-09-24.  Each changed line (33, 48, 75, 77--79) was replaced by
exactly one line, so line numbers cited elsewhere still point to the same text.
\paragraph{C1. The hypothesis of the upper bound (eq.~\eqref{eq:mainbound}, line~48; lines~33, 75 and 77--79).}
\emph{Before:} ``(conditional on \((\mathrm U_\eps)\) for one \(\cA(K)\))''.
\emph{Now:} conditional on hypothesis \((\mathrm V)\) of Definition~\ref{def:V}, which is open.
\emph{Why:} the status table at the end of this note records the upper bound as ``proved under
\((\mathrm V)\)'' (Theorem~\ref{thm:second}, Corollary~\ref{cor:final}), and \((\mathrm U_\eps)\),
\((\mathrm A_\eps)\) of QK for the big interval algebra as refuted (Proposition~\ref{prop:Afails}:
locality in \(K\) forbids them), so the opening named a hypothesis that cannot hold.  \((\mathrm V)\) is
open (Gap~RK1) and holds at \(\sigma=0\) (Proposition~\ref{prop:V0}).  Card \texttt{conj-s2}, Statement:
the upper end ``holds under hypothesis (V), which is open (Gap RK1)''.  \emph{Also:} line~33 named \((\mathrm U_\eps)\) for \(\cN\) as the hypothesis of step~(b); it is \((\mathrm V)\).  Line~75 listed only \((\mathrm A_\eps)\) as failing, and Proposition~\ref{prop:Afails} refutes \((\mathrm U_\eps)\) as well.  Lines~77--79 of the opening table listed \((\mathrm V_\eps)\) at \(\sigma=0\) as proved, by a Lemma \texttt{lem:PW} that this note does not contain, although Proposition~\ref{prop:Afails} shows \(T(\hat g)\Om\notin D(\Delta_K^{-\eps/2})\) for every \(\eps>0\); they now follow the status table: \((\mathrm V)\) at \(\sigma=0\) is proved (Proposition~\ref{prop:V0}), and \((\mathrm V)\) along the orbit is open (Gap~RK1).  Refereed by REF-DOC-OVER.
\par}
