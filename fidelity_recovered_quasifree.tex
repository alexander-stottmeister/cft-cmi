...
\texttt{rate-of-remainder} with \texttt{remainder-c3-c4-values} (C2), \texttt{certified-numerics}
(C3), \texttt{large-zeta-values} (C4) and \texttt{two-chirality-minimum-at-lambda-zero} (C5; both added in a fourth round) of project \texttt{cft\_cmi},
whose refereed Statements are the specification.  C1 follows \texttt{rigor/tail\_bound.tex} as
corrected on the same day (its Corrections C1--C4, C8 and C10).  Each changed line (396, 401, 402; 452 for C4; 437 for C5) was replaced by
...
line~209.
\paragraph{C4. The large-\(\zeta\) brackets (paragraph ``Large cross ratios'', line~452; added in a fourth
round).}
\emph{Before:} ``the box dependence being \(\lesssim10^{-3}\).  Windows up to \(\kappa_c=43.8\) and a
free-exponent extrapolation give the certified brackets of Table~\ref{tab:largezeta}''.
\emph{Now:} the box dependence is \(\lesssim2\times10^{-3}\), and the table gives brackets whose lower ends
are rigorous and whose upper ends are extrapolations, not bounds, together with the best estimates; the
caption of the table was corrected in the same way.
\emph{Why:} the lower end of each bracket, the windowed value \(\Phi_{\rm sub}\) at the largest \(\kappa_c\), is a lower bound by a theorem (\(\Phi_{\rm sub}\le\Phi\) for the sub-compression), though its computed value is not certified either: \(\widehat D\) enters in double precision, as for the small-\(\zeta\) box values (\texttt{rigor/certified\_numerics.tex}, repair~(6)); the upper end \(\Phi_\infty\) is the
extrapolation of the fit \(\Phi_{\rm sub}(\kappa_c)=\Phi_\infty-b\,\kappa_c^{-p}\), and the quoted
uncertainties are the larger of the spread of alternative fits and a quarter of the distance to the lower end (\texttt{numerics/analyze\_largezeta.py}).
At fixed \(\kappa_c\) the boxes \(\Lambda=60,120,180\) differ by up to \(1.6\times10^{-3}\) relative
(\texttt{numerics/results\_largezeta\_certified.txt}, section~3).  Card \texttt{large-zeta-values};
\texttt{open\_problems\_plan.tex}, Correction~C5.
\paragraph{C5. The minimum of the two-chirality law (line~437; added in a fourth round).}
\emph{Before:} ``The \(\lambda\)-dependence is even, with its minimum at \(\lambda=0\), in agreement with the
observation of \cite{VardhanWeiZou} that the fidelity is largest at \(\lambda=0\)''.
\emph{Now:} the dependence is even; the minimum at \(\lambda=0\) is proved only at leading order in
\(\eta_{\mathrm V}\), where the bracket \(2+4\cosh\pi\lambda+2\cosh2\pi\lambda\) is strictly increasing in
\(|\lambda|\) (so for each fixed \(\lambda\neq0\) it holds for all sufficiently small \(\eta_{\mathrm V}\), not
uniformly in \(\lambda\)), and beyond leading order it is a conjecture that agrees with the lattice observation.
\emph{Why:} card \texttt{two-chirality-minimum-at-lambda-zero} (conjectural): ``No proof exists: the only argument
in the documents, rigor/check\_note5\_theory.tex prop:chirality step \(\langle1\rangle8\) \dots, uses
`Phi' nondecreasing', which is false (card phi-derivative-nondecreasing, refuted)''; the minimum would follow from
the weaker, unproved property that \(\zeta\Phi'(\zeta)\) is nondecreasing.  Monotonicity of \(\Phi\) alone does
not give it, since \(\Phi(\zeta_-)\) can only decrease as \(\Phi(\zeta_+)\) increases.  Card
\texttt{two-chirality-law} proves the law and its evenness for the massless Dirac fermion.  The same correction
was made in paper~1 (\texttt{prop:twobranch} and the introduction) and paper~2 (the outlook).
\par}
\end{document}
