...
and no tail bound is needed. `proved`, in shape: whenever `Φ ≤ f ζ²`, the ultraviolet tail obeys
`0 ≤ Φ − Φ_W ≤ C ζ²/κ_c + C′ ζ²/κ_c²` with finite `C`, `C′` for `κ_c ≥ max{10, 2log(1/ζ) + 2log(1+κ_c) + 3}`. The explicit constants `C = 0.173`, `C′ = 0.88` rest on numerical inputs: two suprema of one-dimensional integrals, `A₁ ≤ 2.0` and `A₃ ≤ 0.0374`, evaluated in double precision and not certified, and `f = 0.0085`
([`rigor/tail_bound.tex`](rigor/tail_bound.tex) Cor. 5.2).

`numerical`. The `κ_c → ∞` extrapolation of the window values (rigorous lower bounds; for `f₂` the second-order tail estimate `ζ²/(15κ_c²)`, not a proved bound, is added) gives `f₂ = 0.008444(1)`
