...
\item The one-particle Bures and Kubo--Mori metrics of the recovery curve for the free fermion (\(c=r\)), \(g_B=2r/(3\pi^2)\), \(g_{\mathrm{KM}}=r/6\), by the kernel identity with closed form (P3, refereed by V8); the modular-frame spectral integrals (Note~7, Thm~5.2; V5) and the \(x\)-space Mellin route (P4b) evaluate the same analytic-continuation prescription, so they are checks, not independent routes (Correction~C7).  Hence \(f_2=c/(12\pi^2)\) at the one-particle level; \(s_2=c/12\) is a conjecture, since \(g_{\mathrm{KM}}\) is the metric of the direction, not the coefficient \(s_2\) along the recovery curve.
...
\item The fidelity half of the type-III second-variation lemma for every von Neumann algebra with cyclic separating vector (P1), for curves of vector states satisfying the vector hypothesis (V), norm-differentiable unit vectors; without (V) only \(F=1-O(s)\) follows (Correction~C7).
...
Two-chirality law for 2D CFTs (\(-\log F^{(\lambda)}=\Phi(\zeta_-)+\Phi(\zeta_+)\), even in \(\lambda\), optimum at the ordinary Petz map, in agreement with Vardhan--Wei--Zou; proved only for the massless Dirac fermion, a conjecture for a general 2D CFT, and the optimum at \(\lambda=0\) a conjecture even for the Dirac fermion, Correction~C7); the false \(\Delta^{1/2}\) fidelity formula; the basis-phase bug of the numerics; the stress-tensor normalization chain.
...
\textbf{Definition of done.} A Lamport proof of the tail bound and of the sharp limit, refereed; the certified error bar of the tabulated \(\Phi\) values replaces the heuristic \(0.3\)--\(1\%\).  \emph{Status (2026-09-08): the sharp limit is proved by a different route, Uhlmann's inequality with a Bogoliubov extension of the compression map (\texttt{rigor/uhlmann\_upper\_bound.tex}, refereed), which needs no tail bound and no interchange.  The tail bound itself is proved in shape in \texttt{rigor/tail\_bound.tex}: \(0\le\Phi-\Phi_W\le C\zeta^2/\kappa_c+C'\zeta^2/\kappa_c^2\) with finite \(C,C'\) whenever \(\Phi\le f\zeta^2\), with Prop.~4.1 of Note~5 upgraded to a theorem whose remainder is proved in form; the explicit \(0.173\zeta^2/\kappa_c+0.88\zeta^2/\kappa_c^2\) rests on numerical inputs (\(A_1\le2.0\), \(A_3\le0.0374\) (\(0.036\) before \texttt{rigor/tail\_bound.tex} Correction C10) in double precision, not certified, and \(f=0.0085\)), so the error bars of \texttt{tail\_bound.tex} built on it are not certified (Correction~C2 at the end); after the refinements of \texttt{rigor/certified\_numerics.tex} (weighted Sylvester identity, cross term removed) the a priori bracket \(\Phi_W\le\Phi^{\rm box}\le1.20\,\Phi_W\) holds modulo the \(\widehat D\) quadrature and the box cutoff, with the measured constants \(c_0\in[18,42]\) and \(c_1\le1.011\) (\texttt{certified\_numerics.tex} \S9 (RCr), items (C1), (C2), and its Correction C2), and the 60-digit evaluation (referee-approved arithmetic) gives enclosures of the box value of width \(0.71\)--\(1.01\%\); the \(\widehat D\) quadrature and the box cutoff are the remaining uncertified pieces.  The \(\log^{-2}\) rate claimed in Note~5 is refuted: with the flow extension the Uhlmann bound \(\Phi\le-\frac12\log(1-2f_2\zeta^2)\) holds, even in \(s\), for \(\zeta<\pi\sqrt3\), and up to \(\pi\sqrt6\) modulo an index-zero property that is not yet proved (Correction~C9 at the end), and the expansion is expected, heuristically and numerically but not proved, to proceed in integer powers with the computed \(c_3=-0.99(1)\), \(c_4=+0.9(1)\) (\texttt{rigor/rate\_of\_remainder.tex}; proved is only \(\limsup(\Phi-f_2\zeta^2)/\zeta^3\le0\), Correction~C4 at the end).  The unweighted quadratic square-root bound is false; the weighted Sylvester identity replaces it (\texttt{rigor/certified\_numerics.tex}).}
...
\textbf{Route (revised 2026-09-08 after \texttt{rigor/kubo\_mori\_second\_variation.tex}).} The variational route proposed earlier is refuted: Kosaki's and Petz--Donald's expressions are suprema, so every trial function yields a lower bound, never an upper one.  For unitary orbits with \(A=A^*\) bounded in \(\cM\), or affiliated with \(\cM\) with \(\Omega\in D(A)\) (Correction~C3 at the end), there is instead an exact identity, \(D(\omega\Vert\omega_s)=\|h(\Delta)^{1/2}(e^{isA}-1)\Omega\|^2\) with \(h(\lambda)=\lambda-1-\log\lambda\), from \(\Delta_{u\Omega,\Omega}=u\Delta u^*\) for \(u\in U(\cM)\), together with the sum rule \(\int(\lambda-1)\,d\nu_s=0\) and \(g_{\mathrm{KM}}=2\int h\,d\mu\).  It gives \(\liminf s^{-2}D\ge\frac12g_{\mathrm{KM}}\) with no hypothesis (lower semicontinuity of a closed form), and \(D=\frac{s^2}2g_{\mathrm{KM}}+o(s^2)\) whenever \(\|\Delta^{-\eps/2}(e^{isA}-1)\Omega\|=O(s)\) for some \(\eps>0\), e.g.\ for bounded \(A\) analytic for the modular group in a strip of any width (this implication is proved only for bounded \(A\)).  What remains for Theorem~B is that \(A=T(g_{\rm tot})\) is not affiliated with \(\cA(I)\): a generator affiliated only with a larger algebra is not covered, and that case is open (Gap~QK2 of \texttt{rigor/kubo\_mori\_second\_variation.tex}).  The natural route via restriction from a larger interval algebra \(\cA(K)\) (where the recovery unitary is affiliated) gives an exact unconditional upper bound, but the corresponding gauge lemma is \emph{false}: the infimum over admissible gauges is \((11+5\sqrt5)c/12\approx11.09\cdot c/6\) (\texttt{rigor/kubo\_mori\_gauge\_lemma.tex}); the bracket is \(0.034c\le\liminf s^{-2}D\le\limsup s^{-2}D\le0.924c\), conditional at both ends: the upper half under a uniform-integrability hypothesis that is open (Gap~RK1), the lower half modulo Araki's perturbation theory, whose sources were not obtained (Gap~RK2; Correction~C6).  The value \(c/6\) of Note~7 is the analytic continuation of a divergent integral (the field does not vanish at the moving endpoint); a proof of \(s_2=c/12\) for general nets must renormalise this, e.g.\ through the collar family, or find a route with the sharp cancellation of the free-fermion kernel identity.
...
\textbf{Statement.} (a) Verify on the lattice that the rotated-Petz family of a 2D free-fermion chain obeys \(-\log F^{(\lambda)}=\Phi(z(1+e^{-\pi\lambda}))+\Phi(z(1+e^{\pi\lambda}))\) with the \emph{same} \(\Phi\) as the continuum, including the apparent exponents at \(\lambda=0,0.5,1,1.5\) that reproduce Vardhan--Wei--Zou's fits (done 2026-09-08, \texttt{numerics/lattice}: \(1.99,1.91,1.64,1.15\); two-branch ratio \(1-0.15/L\) constant in \(\eta_{\mathrm V}\); the continuum \(\Phi\) for \(\zeta\ge1\) is now estimated to \(0.5\)--\(2\%\) up to \(\zeta=8.5\) and \(6.6\%\) at \(\zeta=17\), by extrapolation above rigorous windowed lower bounds and not certified (Correction C5; \texttt{numerics/results\_largezeta\_certified.txt}; fitted window exponent drifting from \(2.5\) to \(1.2\)), which also settles the large-cross-ratio law of Note~6 numerically).  (b) Test the single-chirality collapse \(-\log F=\Phi(z(1+e^{-2\pi t}))\) on a chiral lattice model.
...
\textbf{Known.} For a single chirality the geometric compression, the limit of the \emph{rotated} family, is at least as good as every member, and better than the Petz map by a factor \(4\) in \(-\log\Fid\) at leading order; for 2D CFT the Petz map is the optimum of the family at leading order in the cross ratio (beyond it a conjecture, card two-chirality-minimum-at-lambda-zero; Correction~C10); the CMI bound is loose by a power of \(\zeta\) (error \(\sim\zeta^2\) against CMI \(\sim\zeta\)), so no information-theoretic bound presently constrains the optimum.
...
\textbf{Assessment.} Program impact: high (the ``Petz is optimal'' question of Note~3).  General impact: high.  Difficulty: high.  Time: 2--3 months; start after Phase~1.  \emph{Status (2026-09-09, \texttt{rigor/optimality\_second\_order.tex}, \texttt{numerics/optimality}): the second-order problem over quasi-free channels is an exact semidefinite programme in the one-particle data \((X,Z)\) (complete positivity \(\iff Y\ge0,\ XX^*+Y\le1\), verified against a Stinespring dilation); its optimum is \(E_{\rm opt}=(1.00\pm0.15)f_2z^2\) per chirality against \(4f_2z^2\) for the Petz map, i.e.\ exactly the two-chirality geometric-compression value, never below it (lattice: best/Petz \(=0.27\)--\(0.28\)).  The optimal channel is a near-isometric squeeze that does \emph{not} reproduce the \(BC\) marginal, so the defining Petz property is strictly suboptimal.  The lower-bound route through the \(A\)--\(C\) restriction gives exactly zero wherever the intertwiner condition (Tcond) holds (the implication of \texttt{rigor/optimality\_second\_order.tex}, Prop.~prop:AC-zero; then the vacuum \(A\)--\(C\) state is itself \(B\)-mediated), and (Tcond) is verified only numerically, on the three half-filled hopping chains \((L_A,L_B,L_C)=(4,8,2),(4,12,2),(4,16,2)\); at \((6,12,3)\) and \((8,16,4)\) the Moore--Penrose \(T\) violates (Tcond), the direct minimisation gives \(E_{AC}=2.27\times10^{-5}\) at \((6,12,3)\), and nothing is claimed for the continuum or for other geometries (Correction~C1 at the end), and gauge twirling reduces only to gauge-covariant, not to quasi-free, channels: only \(E_{\rm opt}\ge0\) is proved.  Verdict: Petz is not second-order optimal; the geometric compression is consistent with being optimal but unproved (routes: a dual certificate at the saturated constraint \(XX^*=1\); whether every achievable recovered symbol is quasi-free-achievable).}
...
{\sloppy
These corrections implement knowledge-base cards of project \texttt{cft\_cmi}, whose refereed
Statements are the specification: \texttt{ac-lower-bound-zero}, \texttt{tail-bound},
\texttt{km-exact-identity}, (C4) \texttt{rate-of-remainder} with \texttt{remainder-c3-c4-values}, (C5) \texttt{large-zeta-values}, (C6) \texttt{conj-s2}, (C7) \texttt{const-gb}, \texttt{second-variation-lemma} and \texttt{two-chirality-law}, (C8) \texttt{rg-c-function}, (C9) \texttt{rrr-bound-small-zeta} with \texttt{rate-of-remainder}, and (C10) \texttt{two-chirality-law}, \texttt{petz-not-optimal} and \texttt{two-chirality-minimum-at-lambda-zero};
the box bracket in C2 follows card \texttt{certified-numerics}.  The corresponding primary documents were corrected on the same day
(\texttt{rigor/optimality\_second\_order.tex}, Corrections C1 and C2; \texttt{rigor/tail\_bound.tex},
Corrections C1--C4, C8 and C10).  Each changed line was replaced by exactly one line, so line numbers
cited elsewhere still point to the same text.
\paragraph{C1. The \(A\)--\(C\) lower bound (O8, Status, line~288).}
\emph{Before:} ``The lower-bound route through the \(A\)--\(C\) restriction gives exactly zero (the
vacuum \(A\)--\(C\) state is itself \(B\)-mediated)''.
\emph{Now:} it gives exactly zero where the intertwiner condition (Tcond) holds (the implication of
Prop.~\texttt{prop:AC-zero}), and (Tcond) is verified only numerically, on the half-filled hopping
chains \((L_A,L_B,L_C)=(4,8,2)\), \((4,12,2)\), \((4,16,2)\).  At \((6,12,3)\) and \((8,16,4)\) the
Moore--Penrose \(T\) violates it, the direct minimisation gives \(E_{AC}=2.27\times10^{-5}\) at
\((6,12,3)\), and nothing is claimed for the continuum or for other geometries (wording refined in a
third round, at the request of the referee of these corrections).
\emph{Why:} \texttt{rigor/optimality\_second\_order.tex}, Prop.~\texttt{prop:AC-zero}, proves only the
implication (Tcond) \(\Rightarrow E_{AC}=0\).  On the three chains \(T=Q_{AB}^+Q_{AC}\) satisfies
(Tcond) with margins \(0.1216,0.1510,0.1630\) (\texttt{numerics/kb-checks-2026-09-23/tcond\_check.py});
at \((6,12,3)\) and \((8,16,4)\) the margins are \(-0.133\) and \(-0.464\)
(\texttt{tcond\_check\_extra.py}, output \texttt{tcond\_check\_extra.out}); the direct minimisation
\texttt{numerics/optimality/lower\_bound.py} gives \(E_{AC}=2.27\times10^{-5}\) at \((6,12,3)\)
(\texttt{lb.log}).  Card \texttt{ac-lower-bound-zero}, Statement: ``the hypothesis is established
only numerically, on the half-filled hopping chain for \((L_A,L_B,L_C)=(4,8,2),(4,12,2),(4,16,2)\)
\dots\ Nothing is claimed for the continuum or for other geometries: lb.log's third row \((6,12,3)\)
gives \(E_{AC}=2.27\mathrm{e}{-5}\) \dots\ and at \((6,12,3)\) and \((8,16,4)\) the Moore--Penrose \(T\)
violates (Tcond)''.
\paragraph{C2. The tail bound and its error bars (O4, Status, line~209).}
\emph{Before:} ``The tail bound itself is proved in \texttt{rigor/tail\_bound.tex}:
\(0\le\Phi-\Phi_W\le0.173\zeta^2/\kappa_c+0.88\zeta^2/\kappa_c^2\), with Prop.~4.1 of Note~5
upgraded to a theorem with explicit remainder; certified brackets follow''.
\emph{Now:} the shape is proved, \(\Phi-\Phi_W\le C\zeta^2/\kappa_c+C'\zeta^2/\kappa_c^2\) with finite
\(C,C'\) whenever \(\Phi\le f\zeta^2\); the explicit constants \(0.173\) and \(0.88\) rest on the
numerical inputs \(A_1\le2.0\) and \(A_3\le0.0374\) (\(0.036\) before \texttt{rigor/tail\_bound.tex}
Correction C10), evaluated in double precision and not certified, and \(f=0.0085\); so the error bars
of \texttt{tail\_bound.tex}, whose upper ends are that bound, are not certified.  The sentence on
\texttt{rigor/certified\_numerics.tex} that follows concerns a different computation; it called the
a priori bracket \(\Phi_W\le\Phi^{\rm box}\le1.20\,\Phi_W\) ``certified'' and now says that the bracket
holds modulo the \(\widehat D\) quadrature and the box cutoff, with the measured constants
\(c_0\in[18,42]\) and \(c_1\le1.011\) of its hypothesis (\texttt{certified\_numerics.tex} \S9 (RCr),
items (C1) and (C2) of its certification list, and its Correction C2).  These two changes were made in
a third round.
\emph{Why:} card \texttt{tail-bound}, title ``shape proved; explicit constants 0.173, 0.88 rest on
numerical inputs'' and Statement (b): ``\(A_1\le2.0\) and \(A_3\le0.036\), i.e.\ \(C_1\le0.051\) and
\(C_2\le0.46\) (card tail-constants-numerical: uncertified double-precision and finite-difference
evaluations), and \(f=0.0085\)'' (the card's values before \texttt{rigor/tail\_bound.tex} Correction
C10 of 2026-09-24; they are now \(A_3\le0.0374\) and \(C_2\le0.47\), the others unchanged);
\texttt{rigor/tail\_bound.tex}, Corrections C3 and C4 (status of
the constants) and C8 (its bars are \(\Phi^{\rm c}_{\rm sub}+0.173\,\zeta^2/\kappa_{\rm eff}
+0.88\,\zeta^2/\kappa_{\rm eff}^2\), so they inherit these inputs and are no longer called
certified there).
\paragraph{C3. The Kubo--Mori identity for affiliated generators (O2, Route, line~220).}
\emph{Before:} the exact identity was stated ``for unitary orbits with \(A\) affiliated with
\(\cM\)'', and the second-order expansion was said to hold ``e.g.\ for \(A\) analytic for the
modular group in a strip of any width''.
\emph{Now:} the identity, the sum rule and the lower bound are stated for \(A=A^*\) bounded in
\(\cM\), or affiliated with \(\cM\) with \(\Omega\in D(A)\); modular analyticity implies the
condition \(\|\Delta^{-\eps/2}(e^{isA}-1)\Omega\|=O(s)\) only for bounded \(A\); and a generator
affiliated only with a larger algebra, such as \(T(g_{\rm tot})\), is not covered: that case is open
(Gap~QK2).
\emph{Why:} card \texttt{km-exact-identity}, Statement: ``(b) Affiliated generators (prop:unbounded,
l.~774--778): for \(A=A^*\) affiliated with \(M\) and \(\Omega\in D(A)\), the exact identity, the sum
rule, lem:kms and the lower bound hold verbatim, and the second-order expansion holds under
\((U_\eps)\); for unbounded \(A\) the modular-analyticity route \((A_\eps)\Rightarrow(U_\eps)\) is not
available (lem:AtoU is for bounded \(A\)).  (c) Not applicable to the recovery curve of Note~7, whose
generator \(T(g_{\rm tot})\) is affiliated with the global algebra but not with the interval
algebra \dots\ (Gap QK2)''.
\paragraph{C4. The integer-power expansion (O4, Status, line~209; added in a third round).}
\emph{Before:} ``the expansion proceeds in integer powers with \(c_3=-0.99(1)\), \(c_4=+0.9(1)\)
(\texttt{rigor/rate\_of\_remainder.tex}, refereed)''.
\emph{Now:} the expansion is expected, heuristically and numerically but not proved, to proceed in
integer powers with the computed \(c_3=-0.99(1)\), \(c_4=+0.9(1)\); what is proved is
\(\limsup_{\zeta\to0}(\Phi-f_2\zeta^2)/\zeta^3\le0\), from the global bound
\(\Phi\le-\frac12\log(1-2f_2\zeta^2)\) quoted in the same sentence.
...
scripts that produce the values are not in the repository, the existence of the expansion is not
proved, and the lower side is conditional on a cubic constant that is only measured.
\paragraph{C5. The large-\(\zeta\) values (O7, Statement, line~271; added in a fourth round).}
\emph{Before:} ``the continuum \(\Phi\) for \(\zeta\ge1\) is now certified to \(0.5\)--\(2\%\) up to
\(\zeta=8.5\) and \(6.6\%\) at \(\zeta=17\)''.
\emph{Now:} the continuum \(\Phi\) for \(\zeta\ge1\) is estimated to that accuracy, by extrapolation
above rigorous windowed lower bounds, and is not certified.
\emph{Why:} the lower end of each bracket, the windowed value \(\Phi_{\rm sub}\) at the largest \(\kappa_c\), is a lower bound by a theorem (\(\Phi_{\rm sub}\le\Phi\) for the sub-compression), though its computed value is not certified either: \(\widehat D\) enters in double precision, as for the small-\(\zeta\) box values (\texttt{rigor/certified\_numerics.tex}, repair~(6)); the quoted values are the extrapolations
\(\Phi_\infty\) of the fit \(\Phi_{\rm sub}(\kappa_c)=\Phi_\infty-b\,\kappa_c^{-p}\) with free \(p\), and the
percentages are an estimated extrapolation uncertainty (the spread of alternative fits, and at least a
quarter of the distance to the lower end), not an enclosure (\texttt{numerics/results\_largezeta\_certified.txt},
sections~2 and~4; \texttt{numerics/analyze\_largezeta.py}).  Card \texttt{large-zeta-values} (status
numerical) records these values as numerical estimates, and its referee notes that only the lower ends
\(\Phi_{\rm sub}\) are rigorous and that the quoted values are ultraviolet extrapolations.  The same label
was corrected in \texttt{paper1/sec\_numerics.tex}, \texttt{paper1/sec\_2d\_lattice.tex} and
\texttt{paper1/sec\_outlook.tex}; in the caption of \texttt{numerics/fidelity\_table\_largezeta.tex} and its
root copy; in \texttt{fidelity\_recovered\_quasifree.tex} (its Correction~C4); in the labels that
\texttt{numerics/analyze\_largezeta.py} writes into \texttt{numerics/results\_largezeta\_certified.txt}
(numbers unchanged); and by markers in \texttt{numerics/README.md}, \texttt{rigor/findings.tex},
\texttt{rigor/findings\_entries.tex} and \texttt{rigor/PHASE1\_STATUS.md}.
\paragraph{C6. The Kubo--Mori bracket (O2, Route, line~220; added in a fourth round).}
\emph{Before:} ``the proved bracket is \(0.034c\le\liminf s^{-2}D\le\limsup s^{-2}D\le0.924c\), the upper half
under a uniform-integrability hypothesis''.
\emph{Now:} the bracket is conditional at both ends: the upper half under a uniform-integrability
hypothesis that is open, the lower half modulo Araki's perturbation theory, whose sources were not obtained.
\emph{Why:} \texttt{rigor/kubo\_mori\_gauge\_lemma.tex}, status table: the upper end is ``proved under (V)'', and (V) is open (Gap~RK1); the lower end is ``proved modulo Araki's perturbation expansion'' (Thm.~\texttt{thm:lower}, hypothesis~(ii); Gap~RK2: the sources were not obtained, so (ii) is carried as a hypothesis).  Card \texttt{conj-s2}, Statement: ``bracket 0.034 c <= liminf s\^{}-2 D <= limsup <= 0.924 c, conditional at both ends''.  The same correction was made in \texttt{paper2/} (abstract, introduction,
Section~\texttt{sec:km} and the conventions table) and in \texttt{universality\_normalization.tex}
(its Correction~C2).
\paragraph{C7. The summary of the audit (lines~146, 172, 174 and~187; added in a fourth round).}
\emph{Before:} line~146 ``the closed forms \(f_2=c/(12\pi^2)\), \(s_2=c/12\) confirmed by three independent routes,
the lower half of the quadratic law and the fidelity half of the type-III second-variation lemma are proved
unconditionally''; line~172 ``\dots\ by three routes: \dots\ Hence \(f_2=c/(12\pi^2)\), \(s_2=c/12\) at the one-particle level''; line~174 ``The fidelity half of the type-III second-variation lemma for every von Neumann algebra
with cyclic separating vector''; line~187 ``Two-chirality law for 2D CFTs (\dots, optimum at the ordinary Petz map,
\dots)''.
\emph{Now:} \(s_2=c/12\) is a conjecture; of the three computations of the one-particle metrics only the kernel
identity is a proof, at \(c=r\), the other two being checks; the second-variation lemma is proved under the vector
hypothesis (V) of \texttt{rigor/lemma\_second\_variation.tex} (not the hypothesis (V) of C6); the two-chirality law is proved only for the massless Dirac fermion, and its optimum at
\(\lambda=0\) is a conjecture even there.
\emph{Why:} card \texttt{const-gb}: ``Only the kernel-identity computation is rigorous, and only at c = r: the
modular-frame computation \dots\ and the x-space Mellin reassembly \dots\ evaluate the same analytic-continuation
prescription, so they are checks, not independent routes'' and ``g\_KM is the metric of the direction, not the coefficient s2 of the relative entropy along the recovery curve''; card \texttt{conj-s2} (conjectural); card
\texttt{second-variation-lemma}: ``Without (V) the conclusion is not available \dots\ and then only F = 1 - O(s)
follows''; card \texttt{two-chirality-law} (proved, for the massless Dirac fermion): ``NOT claimed here: (i) that the
Dirac error is minimal at lambda = 0 for every z \dots; (ii) the law for a general 2D CFT'', with the conjectural
cards \texttt{two-chirality-minimum-at-lambda-zero} and \texttt{two-chirality-law-general-cft}.
\paragraph{C8. The massive flow (O9, Status, line~295; added in a fourth round).}
\emph{Before:} ``\(c_M/c_E\to mR\)''.
\emph{Now:} \(c_M/c_E\) behaves like \(mR\) only in a scaling window; at fixed lattice mass \((c_M/c_E)/(mR)\) does not tend
to \(1\), and the data are only consistent with \(c_M/c_E\sim mR\) in the continuum scaling limit, which they do not reach.
\emph{Why:} card \texttt{rg-c-function} (numerical), Statement: ``The ratio c\_M/c\_E behaves like m R only in a scaling
window \dots; on the lattice at fixed m the ratio (c\_M/c\_E)/(mR) does not tend to 1: for m = 0.4 it is 1.196, 1.077,
1.020, 1.003, 0.992, 0.972 at R = 6, 10, 16, 20, 24, 40 and still falling, and at R = 40 it is 0.825 for m = 0.8 \dots\ This
is consistent with c\_M/c\_E \(\sim\) m R in the continuum scaling limit \dots, which the data do not reach''.
\paragraph{C9. The range of the remainder bound (O4, Status, line~209; added in a fourth round).}
\emph{Before:} ``with the flow extension the Uhlmann bound is global, \(\Phi\le-\frac12\log(1-2f_2\zeta^2)\), even in \(s\)''.
\emph{Now:} the bound holds for \(\zeta<\pi\sqrt3=5.44\); on \(\pi\sqrt3\le\zeta<\pi\sqrt6\) it rests on index zero of the
diagonal block, i.e.\ on the equality of the singular values of the two off-diagonal blocks, which is not yet proved
there.  The rate statements of the same sentence use only small \(\zeta\) and are unaffected.
\emph{Why:} card \texttt{rrr-bound-small-zeta} (proved): the bound for \(0\le\zeta<(4f_2)^{-1/2}=\pi\sqrt3\); card
\texttt{rate-of-remainder}: ``for pi sqrt(3) <= zeta < (2 f2)\^{}\{-1/2\} = pi sqrt(6) = 7.70 in the bound with f2, they
rest on the cards pushforward-blocks-equal-singular-values (heuristic) \dots\ and group-index-zero-large-s
(conjectural)''; \texttt{rigor/rate\_of\_remainder.tex}, Correction C3 (a known gap, not corrected there).
\paragraph{C10. Optimality within the rotated family (O8, Known, line~282; added in a fourth round).}
\emph{Before:} ``For a single chirality the geometric compression is the optimum of the \emph{rotated} family, better than
the Petz map by a factor \(4\) in \(-\log\Fid\); for 2D CFT the Petz map is the optimum of the family''.
\emph{Now:} the geometric compression is the limit of the rotated family, at least as good as every member, and better
than the Petz map by a factor \(4\) at leading order; for 2D CFT the Petz map is the optimum of the family at leading
order in the cross ratio, and beyond it a conjecture.
\emph{Why:} card \texttt{two-chirality-law}: the geometric compression, ``the t -> +infinity endpoint, which is a limit
point and not a member of the family, is at least as good as every member''; card \texttt{petz-not-optimal}: ``at leading
order the Petz map is worse by the factor 4''; card \texttt{two-chirality-minimum-at-lambda-zero} (conjectural).
\par}
\end{document}
