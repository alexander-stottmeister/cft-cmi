...
$V_\zeta=P_-\widetilde W_{\varphi_s}P_+$, and the same argument for the other block $P_+\widetilde W_{\varphi_s}P_-=(P_-\widetilde W_{\varphi_s}^*P_+)^*$, give $[P_+,\widetilde W_{\varphi_s}]$ Hilbert--Schmidt and hence implementability \cite[Cor.~2.7]{RigorUB}; the Shale--Stinespring criterion needs both off-diagonal blocks, and $V_\zeta$ alone does not suffice.
\item \emph{Uhlmann inequality.}  By \cref{lem:vector} and
\cite[Thm.~2(3)]{AU02}, $\Fid\ge|\langle\Omega,\Gamma(U)\Omega\rangle|$ for
$U=\widetilde W_{\varphi_s}\widehat W'$ with any unitary $\widehat W'=\one_{L^2(I)}\oplus W'$ localized in $I'$ whose two off-diagonal blocks $P_-\widehat W'P_+$ and $P_+\widehat W'P_-$ are both Hilbert--Schmidt, so that an implementer $\Gamma(\widehat W')$ exists (the first block alone does not suffice) \cite[Lem.~3.2]{RigorUB},
