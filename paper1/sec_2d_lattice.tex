...
\end{equation}
For $c=\bar c$ this is even in $\lambda$ (a unique minimum at
$\lambda=0$ is expected but proved only at leading order, see the end of the proof), and
...
\end{equation}
with $f_2=c/(12\pi^2)$ per chiral component.  The relative entropy is expected to obey the same
law with $\Phi$ replaced by $\cD$ and $f_2$ by $s_2$, conjecturally $c/12$ (the relative entropy follows it numerically to $0.5\%$), i.e.\
$D=\frac{c+\bar c}3\eta_{\rm V}^2+\dots$ at $\lambda=0$ if $s_2=c/12$.
\end{proposition}
...
$8f_2=2c/(3\pi^2)$ per chirality.  At leading order the minimum is at $\lambda=0$, because
$\lambda\mapsto2+4\cosh\pi\lambda+2\cosh2\pi\lambda$ is strictly increasing in
$|\lambda|$, so for each fixed $\lambda\neq0$ it holds for all sufficiently small $z$, though not uniformly in $\lambda$.  Beyond leading order it is not proved: $\Phi$ is nondecreasing
(\cref{prop:monotone}), but as the pair $(\zeta_-,\zeta_+)$ moves outward from the
symmetric point $\Phi(\zeta_-)$ can only decrease while $\Phi(\zeta_+)$ can only increase; the minimum would follow if $\zeta\Phi'(\zeta)$ were nondecreasing, which is not proved.
\end{proof}
...
$\zeta=1.067,\,2.133,\,4.267,\,8.533,\,17.07$; the extrapolations (estimates, not bounds) of
