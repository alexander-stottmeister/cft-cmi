...
The argument in \cite{Note5} is a sketch; a complete proof with explicit remainder,
$|\Sigma(\kappa_c)-\zeta^2/(15\kappa_c^2)|\le0.46\,\zeta^2\kappa_c^{-3}+0.051\,\zeta(1+\kappa_c)e^{-\kappa_c}$,
together with the unconditional full-tail bound $0\le\Phi-\Phi_W\le0.173\,\zeta^2/\kappa_c+0.88\,\zeta^2/\kappa_c^2$
for $\kappa_c\ge\max\{10,2\log(1/\zeta)+2\log(1+\kappa_c)+3\}$, is given in \cite{RigorTB}.
...
\cref{sec:quadratic}; the cubic coefficient is established numerically \cite{RigorRR}.  For positive collar
