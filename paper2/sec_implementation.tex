...
push-forward, implementable by Shale--Stinespring \cite{SS} because both off-diagonal blocks, $\Pi_{-}W_{s}\Pi_{+}$ and $\Pi_{+}W_{s}\Pi_{-}=(\Pi_{-}W_{s}^{*}\Pi_{+})^{*}$, are
Hilbert--Schmidt \cite{RigorUB} (the criterion needs both blocks, i.e.\ $[\Pi_{+},W_{s}]$ Hilbert--Schmidt; the first block alone does not suffice).  That argument needs much less than $C^{1,1}$ --- essentially the
